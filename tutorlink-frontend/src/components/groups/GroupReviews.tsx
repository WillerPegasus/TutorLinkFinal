import { GroupReview } from '../../types/group.types';

interface Props { reviews: GroupReview[]; count: number; }

const Stars = ({ n }: { n: number }) => (
  <span className="text-yellow-400 text-sm">{'★'.repeat(n)}{'☆'.repeat(5-n)}</span>
);

const GroupReviews = ({ reviews, count }: Props) => (
  <div className="bg-white rounded-xl shadow-sm p-5">
    <h3 className="font-bold text-gray-700 mb-4">⭐ Avis des élèves inscrits ({count})</h3>
